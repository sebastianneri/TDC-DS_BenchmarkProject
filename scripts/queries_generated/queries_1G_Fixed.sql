...
(select w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
 ...
 from(select w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy
      ...
      group by w_warehouse_name,w_warehouse_sk,i_item_sk,d_moy) foo
 where case mean when 0 then 0 else stdev/mean end > 1)
select inv1.w_warehouse_sk wrsk,inv1.i_item_sk _itemk,inv1.d_moy d_moy,inv1.mean mean, inv1.cov cov
        ,inv2.w_warehouse_sk wrsk2,inv2.i_item_sk _itemk2,inv2.d_moy d_moy2,inv2.mean mean2, inv2.cov cov2
...
select  s_store_name, s_store_id,
 ...
 from $date_dim$, $store_sales$, $store$  where d_date_sk = ss_sold_date_sk and
...
with v1 as(
 ...
 from $item$, $store_sales$, $date_dim$, $store$  where ss_item_sk = i_item_sk and
       ss_sold_date_sk = d_date_sk and
       ss_store_sk = s_store_sk and
       (
         d_year = 2000 or
...
with ss as
 ...
 from $store_sales$,
      $date_dim$,
      $store$  where ss_sold_date_sk = d_date_sk
 ...
 from $store_returns$,
      $date_dim$,
      $store$  where sr_returned_date_sk = d_date_sk
       and d_date between cast('2000-08-10' as date)
                  and date_add(cast('2000-08-10' as date),30)
...
from
 ...
 from $store_sales$, $household_demographics$ ,  $time_dim$ , $store$  where ss_sold_time_sk =  $time_dim$ .t_time_sk   
...
from $store_sales$
    ,$household_demographics$ 
    , $time_dim$ , $store$ where ss_sold_time_sk =  $time_dim$ .t_time_sk   
    and ss_hdemo_sk = $household_demographics$.hd_demo_sk 
    and ss_store_sk = s_store_sk
    and  $time_dim$ .t_hour = 8
    and  $time_dim$ .t_minute >= 30
